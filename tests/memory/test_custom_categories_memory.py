...
class CustomCategoriesTester:
    ...
    def test_search_with_categories_filter(self) -> bool:
        ...
        # 使用 search API，在 filters 中指定 categories
        body = {
            "query": "food",
            "user_id": self.TEST_USER_ID,
            "filters": {
    ...
    def test_search_with_multiple_categories_filter(self) -> bool:
        ...
        # 使用 search API，在 filters 中指定多个 categories（OR 逻辑）
        body = {
            "query": "food",
            "user_id": self.TEST_USER_ID,
            "filters": {
    ...
    def test_get_memories_with_categories_icontains_filter(self) -> bool:
        ...
        for mem in results:
            ...
            cats_str = meta.get("categories", "")
            if "food" in cats_str.lower():
                print(f"  ✓ 记忆 {mem.get('id', '')[:12]}... categories 包含 'food' 子串 (categories={cats_str})")
            else:
                print(f"  ✗ 记忆 {mem.get('id', '')[:12]}... categories 不包含 'food' 子串 (categories={cats_str})")
                all_match = False
